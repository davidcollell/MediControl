  ...
  const getTodaysTasks = (): DoseTask[] => {
    ...
    medications.forEach(med => {
      ...
      validSchedules.forEach(schedule => {
        ...
        // Utilitzem l'hora programada (scheduledTime) al log per diferenciar dosis
        const isTaken = logs.some(l => 
          l.medicationId === med.id && 
          l.status === 'taken' && 
          (l.scheduledTime === schedule.time || !l.scheduledTime /* fallback per legacy */)
        );
      ...
      <header className="flex flex-col gap-4 pt-2">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            Avui <span className="text-slate-400 font-light">|</span> <span className="text-sky-700 capitalize">{formattedDate}</span>
          </h1>
        </div>
      ...
      <div className="space-y-6">
        ...
        {pendingTasks.length === 0 && totalDaily > 0 ? (
          <div className="text-center py-10 bg-green-50 rounded-3xl border-2 border-green-100">
            ...
            <p className="text-xl font-bold text-green-800">Fantàstic!</p>
            <p className="text-green-700">Has pres tota la medicació d'avui.</p>
          </div>
